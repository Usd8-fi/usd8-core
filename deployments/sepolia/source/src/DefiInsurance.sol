    ...
    SharedBase
{
    ...
    event IncidentOpened(uint256 indexed incidentId, IERC20 indexed insuredToken, uint64 claimDeadline);
    ...
    function editInsuredToken(
        ...
        if (maxCoverageBps > BPS_DENOMINATOR) {
            revert InvalidMaxCoverageBps(maxCoverageBps, BPS_DENOMINATOR);
        }
        if (isNew) {
            // Add a new token.
        ...
        returns (uint256 incidentId)
    {
        ...
        emit IncidentOpened(incidentId, insuredToken, claimDeadline);
    }
    ...
    function _payClaimAmounts(uint256 incidentId, uint256[] calldata amounts) internal {
        Incident storage inc = incidents[incidentId];
        address[] storage poolAddrs = inc.pools;
        uint256[] storage budget = inc.poolBudget;
        for (uint256 i = 0; i < poolAddrs.length; i++) {

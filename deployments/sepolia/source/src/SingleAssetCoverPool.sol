...
/// @notice ERC-4626 vault for one cover asset. Depositors receive transferable
///         shares, earn streamed USD8 rewards, and share insurance losses.
/// @dev Exits are asynchronous. {requestRedeem} rounds the cooldown end to a batch
///      boundary, then clamps it to the outstanding queue tail to preserve FIFO order.
///      Escrowed shares stop earning but remain loss-exposed until permissionless epoch
///      settlement fixes their asset value and burns them. {completeRedeem} has no expiry.
///      Active incidents block deposits and epoch settlement, but not exit requests,
///      share transfers, or claims from already reserved exits. {totalAssets} uses
///      internal accounting, so donations do not inflate the share price.
    ...
    IProfitDistributionReceiver
{
    ...
    /// @notice Maximum reward schedule length; limits zero-rate and overflow cases.
    uint64 public constant MAX_REWARDS_DURATION = 365 days;

    /// @notice Fixed-point scale for reward-per-share accounting.
    ...
    /// @param exitEpoch Assigned exit-batch boundary.
    struct ExitRequest {
        uint256 shares;
        uint64 exitEpoch;
    }
    ...
    ///         until their assigned epoch settles after the cooldown. One live request
    ...
    /// @dev Rounds the earliest exit time up to the next batch boundary, then
    ///      preserves FIFO ordering against the outstanding queue tail.
    function _calculateExitEpoch(uint256 requestedAt) internal view returns (uint64 exitEpoch) {
        Registry.ExitTimingConfig memory config = registry().exitTimingConfig();
        uint256 earliest = requestedAt + config.unstakeCooldown;
        uint256 interval = config.exitBatchInterval;
        exitEpoch = SafeCast.toUint64(Math.ceilDiv(earliest, interval) * interval);

        uint256 length = _exitEpochQueue.length;
        if (nextExitEpochIndex < length) {
            uint64 outstandingTail = _exitEpochQueue[length - 1];
            if (exitEpoch < outstandingTail) exitEpoch = outstandingTail;
        }
    }
    ...
    function completeRedeem(address receiver) external nonReentrant returns (uint256 assets) {
        _requireNotPaused();
    ...
    /// @notice Pull USD8 from the caller and blend it into the current reward stream.
    /// @dev Reverts if no shares can earn or if the resulting rate rounds to zero.
    ///      Small division remainders stay in the reward reserve.
    ...
    /// @notice Current incident cap: {totalAssets} × {Registry.maxCoverPoolPayoutBps}
    ///         / 10_000. DefiInsurance checks it when setting the pool budget.
    ...
    /// @notice Set the emission window for future distributions. Timelock only.
    function setRewardsDuration(uint64 newDuration) external {
        _requireTimelock();
    ...
    /// @dev Checkpoint global rewards before earning shares change. With no earning
    ///      shares, move the unstreamed window forward so rewards remain deferred.
    ...
    /// @dev Accrue one account through the current global checkpoint.

...
function rpc(): string {
  ...
  if (!u) throw new Error("RPC_URL not set");
  return u;
}

async function buildSettlement(incidentId: bigint): Promise<Settlement> {
  ...
  if (localHash.toLowerCase() !== onchainInputHash.toLowerCase()) {
    throw new Error(
      `inputHash mismatch: local ${localHash} vs onchain ${onchainInputHash} — table reconstruction is wrong`
    );
  }
...
function printTable(s: Settlement) {
  const out = {
...
async function main() {
  ...
  if (mode === "keygen") {
    ...
    if (process.env.PRINT_KEY === "1") console.log(pk);
    return;
  }
  ...
  if (mode === "settle") {
    const key = process.env.SIGNER_KEY as `0x${string}` | undefined;
    if (!key) throw new Error("SIGNER_KEY not set (enclave-held key)");

...
export const CLAIM_REGISTERED = parseAbiItem(
  "event ClaimRegistered(uint256 indexed claimId, uint256 indexed incidentId, address indexed user, uint128 insuredTokenAmount)"
);
export const CLAIM_CANCELLED = parseAbiItem(
...
export function liveTable(events: InputEvent[]): TableEntry[] {
 ...
 */
export async function feedUsd1e18(
  client: PublicClient,
  feed: `0x${string}`,
  blockNumber: bigint
): Promise<bigint> {
  ...
  if (answer <= 0n) throw new Error(`feed ${feed} returned non-positive price`);
...
export async function minBalanceOver(
  ...
  const events = [...outs.map((l) => ({ l, sign: -1n })), ...ins.map((l) => ({ l, sign: 1n }))].sort(
    (a, b) =>
 ...
 */
export async function twabOver(
  client: PublicClient,
  token: `0x${string}`,
  who: `0x${string}`,
  fromBlock: bigint,

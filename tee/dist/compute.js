...
import { ratioAt, feedUsd1e18, minBalanceOver, twabOver, } from "./chain.js";
const WAD = 10n ** 18n;
const BPS = 10000n;
/**
 ...
 */
export function computeInputHash(events) {
    let h = `0x${"00".repeat(32)}`;
    for (const e of events) {
        if (e.kind === "register") {
            h = keccak256(encodeAbiParameters([{ type: "bytes32" }, { type: "uint256" }, { type: "address" }, { type: "uint128" }], [h, e.claimId, e.user, e.amount]));
        }
        else {
            h = keccak256(encodeAbiParameters([{ type: "bytes32" }, { type: "uint256" }, { type: "string" }], [h, e.claimId, "CANCEL"]));
        }
    }
    return h;
}
 ...
 */
export async function detectIncidentBlock(client, cfg, firstClaimBlock, windowEndBlock) {
...
export async function twapRatioBefore(client, cfg, b) {
 ...
 */
export async function settle(client, incidentId, cfg, events, opts) {
    ...
    if (!cliff)
        throw new Error("no qualifying ratio cliff: incident invalid, do not sign");
    ...
    const scoreFrom = opts.windowEndBlock > scoreBlocks ? opts.windowEndBlock - scoreBlocks : 1n;
    ...
    for (const e of live) {
        ...
        const lossUsd = (((eligible * twap) / WAD) * underlyingUsd) / 10n ** BigInt(cfg.underlyingDecimals);
    ...
    for (const r of rows) {
        ...
        r.amounts = opts.assetOrder.map((_, i) => poolUsd === 0n ? 0n : (r.payoutUsd * opts.assetBalances[i]) / poolUsd);
    }
    ...
    const tree = StandardMerkleTree.of(rows.map((r) => [incidentId, r.claimId, r.user, r.amounts]), ["uint256", "uint256", "address", "uint256[]"]);
    return {
        incidentId,
...
export function proofFor(s, claimId) {
    const tree = StandardMerkleTree.of(s.rows.map((r) => [s.incidentId, r.claimId, r.user, r.amounts]), ["uint256", "uint256", "address", "uint256[]"]);

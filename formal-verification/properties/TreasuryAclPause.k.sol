...
contract TreasuryAclPauseKontrolTest is Test {
    ...
    uint256 internal constant CUSTOM_DIVISOR = 2000;
    ...
    // [C:ADDRESS_REPRESENTATIVE] OUTSIDER represents every address unequal to
    // the Registry's admin and timelock; each ACL check depends only on those equalities.
    function test_unauthorizedCallerCannotAddOrRemoveStrategiesAtomically() public {
        ...
        (bool addSuccess, bytes memory addData) =
            _callAs(OUTSIDER, abi.encodeCall(Treasury.addStrategy, (IStrategy(address(candidate)), uint256(0))));
        assert(!addSuccess);
        ...
        (bool removeSuccess, bytes memory removeData) =
            _callAs(OUTSIDER, abi.encodeCall(Treasury.removeStrategy, (IStrategy(address(approved)))));
    ...
    function test_unauthorizedCallerCannotDepositWithdrawOrHarvestAtomically(uint64 amount) public {
        vm.assume(amount > 0);
...

        {
            (bool success, bytes memory data) = _callAs(
                OUTSIDER, abi.encodeCall(Treasury.depositToStrategy, (IStrategy(address(strategy)), uint256(amount)))
            );
            assert(!success);
        ...
        {
            (bool success, bytes memory data) = _callAs(
                OUTSIDER, abi.encodeCall(Treasury.withdrawFromStrategy, (IStrategy(address(strategy)), uint256(amount)))
            );
            assert(!success);
            assert(_selector(data) == Registry.UnauthorizedAdmin.selector);
        }
        {
            (bool success, bytes memory data) = _callAs(OUTSIDER, abi.encodeCall(Treasury.harvestAndDistribute, ()));
            assert(!success);
            assert(_selector(data) == Registry.UnauthorizedAdmin.selector);
        }
    ...
    function test_unauthorizedCallerCannotSetOrRemoveReceiverAtomically(uint64 weight) public {
        ...
        (bool removeSuccess, bytes memory removeData) =
            _callAs(OUTSIDER, abi.encodeCall(Treasury.removeProfitReceiver, (address(existing))));
        assert(!removeSuccess);
    ...
    function test_unauthorizedCallerCannotSweepTokenOrETHAtomically(uint128 tokenAmount, uint128 ethAmount) public {
        ...
        (bool tokenSuccess, bytes memory tokenData) =
            _callAs(OUTSIDER, abi.encodeCall(SharedBase.sweepToken, (IERC20(address(foreignToken)), RECIPIENT)));
        assert(!tokenSuccess);
        assert(_selector(tokenData) == Registry.UnauthorizedAdmin.selector);

        (bool ethSuccess, bytes memory ethData) =
            _callAs(OUTSIDER, abi.encodeCall(SharedBase.sweepETH, (payable(RECIPIENT))));
        assert(!ethSuccess);
    ...
    function test_unauthorizedCallerCannotUpgradeAtomically() public {
        TreasuryAclPauseV2 candidate = new TreasuryAclPauseV2();
        bytes32 implementationBefore = _implementationWord();

        (bool success, bytes memory returndata) =
            _callAs(OUTSIDER, abi.encodeCall(UUPSUpgradeable.upgradeToAndCall, (address(candidate), bytes(""))));
        assert(!success);
    ...
    function test_harvestBufferDivisorDefaultsAndIsTimelockOnly(uint256 divisor) public {
        vm.assume(divisor != 0);
        assert(treasury.harvestBufferDivisor() == treasury.HARVEST_BUFFER_DIVISOR());

        (bool adminSuccess, bytes memory adminData) =
            _callAs(ADMIN, abi.encodeCall(Treasury.setHarvestBufferDivisor, (divisor)));
        assert(!adminSuccess && _selector(adminData) == Registry.UnauthorizedTimelock.selector);
        assert(treasury.harvestBufferDivisor() == treasury.HARVEST_BUFFER_DIVISOR());

        (bool zeroSuccess, bytes memory zeroData) =
            address(treasury).call(abi.encodeCall(Treasury.setHarvestBufferDivisor, (uint256(0))));
        assert(!zeroSuccess);
        assert(
            keccak256(zeroData)
                == keccak256(abi.encodeWithSelector(Treasury.InvalidHarvestBufferDivisor.selector, uint256(0)))
        );
        assert(treasury.harvestBufferDivisor() == treasury.HARVEST_BUFFER_DIVISOR());

        vm.expectEmit(false, false, false, true, address(treasury));
        emit Treasury.HarvestBufferDivisorSet(treasury.HARVEST_BUFFER_DIVISOR(), divisor);
        treasury.setHarvestBufferDivisor(divisor);
        assert(treasury.harvestBufferDivisor() == divisor);

        vm.expectEmit(false, false, false, true, address(treasury));
        emit Treasury.HarvestBufferDivisorSet(divisor, CUSTOM_DIVISOR);
        treasury.setHarvestBufferDivisor(CUSTOM_DIVISOR);
        assert(treasury.harvestBufferDivisor() == CUSTOM_DIVISOR);
    }
    ...
    function test_pauseIntentionallyAllowsStrategyAndReceiverCuration() public {
    ...
    function test_unpauseRestoresRedeemWithdrawalAndHarvest() public {
        ...
        uint256 expectedHarvest =
            reserveBeforeHarvest * SCALE - supplyBeforeHarvest - supplyBeforeHarvest / treasury.harvestBufferDivisor();
        (uint256 harvested, uint256 distributed) = treasury.harvestAndDistribute();

...
contract USD8SweepKontrolTest is Test {
    ...
    // [C:ADDRESS_REPRESENTATIVE] OUTSIDER represents every caller unequal to
    // the Registry's admin and timelock; the ACL depends only on those equalities.
    function test_unauthorizedCallerCannotSweepTokenAtomically(uint128 amount) public {
        vm.assume(amount > 0);
    ...
    function test_unauthorizedCallerCannotSweepETHAtomically(uint128 amount) public {
        vm.assume(amount > 0);

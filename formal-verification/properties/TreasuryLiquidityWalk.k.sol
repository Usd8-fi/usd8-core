...
contract TreasuryLiquidityWalkKontrolTest is Test {
    ...
    // [C:MOCK_CALL_SEQUENCE] Kontrol v1.0.255 does not implement vm.mockCalls.
    // These call-indexed adversarial schedules remain executable Foundry
    // regressions in TreasuryLiquidityWalkCallIndexedForgeTest below.
    function _checkCallIndexedPrecheckValuationDecodeRevertIsAtomic() internal {
        _seedAndAllocate(10 * uint64(UNIT), 10 * uint64(UNIT), 0, 0);
        uint256 supplyBefore = usd8.totalSupply();
    ...
    function _checkCallIndexedPayoutValuationDecodeRevertIsAtomic() internal {
        _seedAndAllocate(10 * uint64(UNIT), 10 * uint64(UNIT), 0, 0);
        uint256 supplyBefore = usd8.totalSupply();
        uint256 userBefore = usd8.balanceOf(address(user));
        uint256 strategyBefore = usdc.balanceOf(address(strategy0));
    ...
    function _checkCallIndexedWalkValuationDecodeRevertRollsBackBurn() internal {
        _seedAndAllocate(10 * uint64(UNIT), 10 * uint64(UNIT), 0, 0);
        uint256 supplyBefore = usd8.totalSupply();
        uint256 userBefore = usd8.balanceOf(address(user));
        uint256 strategyBefore = usdc.balanceOf(address(strategy0));
    ...
    function _checkCallIndexedWalkUnderreportChangesToPullThenExhaustionIsAtomic() internal {
        _seedAndAllocate(10 * uint64(UNIT), 10 * uint64(UNIT), 0, 0);
        uint256 supplyBefore = usd8.totalSupply();
        uint256 userBefore = usd8.balanceOf(address(user));
        uint256 strategyBefore = usdc.balanceOf(address(strategy0));
    ...
    function _checkCallIndexedPayoutDriftChangesExactPayoutAndCompletesFourMeasuredStages() internal {
        _seedAndAllocate(10 * uint64(UNIT), 10 * uint64(UNIT), 0, 0);
    ...
    function _checkCallIndexedPostcheckValuationDecodeRevertRollsBackPayoutAndPull() internal {
        _seedAndAllocate(10 * uint64(UNIT), 10 * uint64(UNIT), 0, 0);
        uint256 supplyBefore = usd8.totalSupply();
        uint256 userBefore = usd8.balanceOf(address(user));
        uint256 strategyBefore = usdc.balanceOf(address(strategy0));
    ...
    function _checkCallIndexedPostcheckDownwardDriftRevertsWholeSuccessfulWalk() internal {
        _seedAndAllocate(10 * uint64(UNIT), 10 * uint64(UNIT), 0, 0);
        uint256 supplyBefore = usd8.totalSupply();
        uint256 userBefore = usd8.balanceOf(address(user));
        uint256 strategyBefore = usdc.balanceOf(address(strategy0));
...
/// @notice Foundry-only call-indexed adversarial schedules. Kontrol v1.0.255
///         reports CHEATCODE_UNIMPLEMENTED for vm.mockCalls.
contract TreasuryLiquidityWalkCallIndexedForgeTest is TreasuryLiquidityWalkKontrolTest {
    function test_callIndexedPrecheckValuationDecodeRevertIsAtomic() public {
        _checkCallIndexedPrecheckValuationDecodeRevertIsAtomic();
    }

    function test_callIndexedPayoutValuationDecodeRevertIsAtomic() public {
        _checkCallIndexedPayoutValuationDecodeRevertIsAtomic();
    }

    function test_callIndexedWalkValuationDecodeRevertRollsBackBurn() public {
        _checkCallIndexedWalkValuationDecodeRevertRollsBackBurn();
    }

    function test_callIndexedWalkUnderreportChangesToPullThenExhaustionIsAtomic() public {
        _checkCallIndexedWalkUnderreportChangesToPullThenExhaustionIsAtomic();
    }

    function test_callIndexedPayoutDriftChangesExactPayoutAndCompletesFourMeasuredStages() public {
        _checkCallIndexedPayoutDriftChangesExactPayoutAndCompletesFourMeasuredStages();
    }

    function test_callIndexedPostcheckValuationDecodeRevertRollsBackPayoutAndPull() public {
        _checkCallIndexedPostcheckValuationDecodeRevertRollsBackPayoutAndPull();
    }

    function test_callIndexedPostcheckDownwardDriftRevertsWholeSuccessfulWalk() public {
        _checkCallIndexedPostcheckDownwardDriftRevertsWholeSuccessfulWalk();
    }
}

...
contract TreasuryHarvestDirectKontrolTest is Test {
    ...
    function test_customDivisorExactUnderBufferEndpointMintsNothing(uint32 bufferReserve, uint32 divisor) public {
        vm.assume(bufferReserve > 0 && divisor > 0 && divisor != treasury.HARVEST_BUFFER_DIVISOR());
        uint256 backing = uint256(bufferReserve) * divisor;
        _mintSupply(backing);
        treasury.setHarvestBufferDivisor(divisor);
        usdc.mint(address(treasury), uint256(bufferReserve) - 1);
        uint256 supplyBefore = usd8.totalSupply();
        uint256 reserveBefore = treasury.getReserveBalance();
        uint256 retain = supplyBefore + supplyBefore / divisor;
        assert(reserveBefore * SCALE + SCALE == retain);

        (uint256 harvested, uint256 distributed) = treasury.harvestAndDistribute();

        assert(harvested == 0 && distributed == 0);
        assert(usd8.totalSupply() == supplyBefore);
        assert(treasury.getReserveBalance() == reserveBefore);
        assert(treasury.harvestBufferDivisor() == divisor);
    }

    function test_customDivisorExactEqualBufferEndpointMintsNothing(uint32 bufferReserve, uint32 divisor) public {
        vm.assume(bufferReserve > 0 && divisor > 0 && divisor != treasury.HARVEST_BUFFER_DIVISOR());
        uint256 backing = uint256(bufferReserve) * divisor;
        _mintSupply(backing);
        treasury.setHarvestBufferDivisor(divisor);
        ...
        uint256 supplyBefore = usd8.totalSupply();
        uint256 reserveBefore = treasury.getReserveBalance();
        assert(reserveBefore * SCALE == supplyBefore + supplyBefore / divisor);
    ...
    function test_customDivisorExactOverBufferEndpointHarvestsOneReserveUnit(uint32 bufferReserve, uint32 divisor)
        public
    {
        vm.assume(bufferReserve > 0 && divisor > 0 && divisor != treasury.HARVEST_BUFFER_DIVISOR());
        uint256 backing = uint256(bufferReserve) * divisor;
        ...
        uint256 supplyBefore = usd8.totalSupply();
        uint256 reserveBefore = treasury.getReserveBalance();
        assert(reserveBefore * SCALE == supplyBefore + supplyBefore / divisor + SCALE);

        (uint256 harvested, uint256 distributed) = treasury.harvestAndDistribute();

        assert(harvested == SCALE && distributed == SCALE);
        assert(usd8.totalSupply() == supplyBefore + SCALE);
        assert(usd8.balanceOf(RECEIVER_0) == SCALE);
        assert(treasury.getReserveBalance() == reserveBefore);
        assert(treasury.harvestBufferDivisor() == divisor);
    }
    ...
    function test_profitReceiverWeightAboveUint88RevertsBeforeRegistration() public {
        ...
        (bool success, bytes memory returndata) = address(treasury)
            .call(
                abi.encodeCall(
                    ...
                    (RECEIVER_0, oversizedWeight, Treasury.RevenueDistributionMode.DirectTransfer)
                )
            );

        assert(!success);
        assert(_selector(returndata) == SafeCast.SafeCastOverflowedUintDowncast.selector);
        assert(returndata.length == 68);
        uint256 encodedBits;
        uint256 encodedValue;
        assembly {
            encodedBits := mload(add(returndata, 0x24))
            encodedValue := mload(add(returndata, 0x44))
        }
        assert(encodedBits == 88);
        assert(encodedValue == oversizedWeight);

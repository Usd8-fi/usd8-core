...
contract USD8TokenKontrolTest is Test {
    ...
    function test_treasuryIsOneTimeAndMintAuthorityStaysCanonical(address recipient, uint128 mintAmount) public {
        vm.assume(recipient != address(0));
        vm.assume(mintAmount > 0);

        (bool setSuccess, bytes memory setReturndata) =
            address(registry).call(abi.encodeCall(Registry.setTreasury, (address(otherCaller))));
        assert(!setSuccess);
        assert(_selector(setReturndata) == Registry.TreasuryAlreadySet.selector);
        assert(usd8.treasury() == address(this));

        uint256 supplyBefore = usd8.totalSupply();
        uint256 balanceBefore = usd8.balanceOf(recipient);

        (bool replacementSuccess, bytes memory replacementReturndata) =
            otherCaller.callMint(usd8, recipient, mintAmount);
        assert(!replacementSuccess);
        assert(_selector(replacementReturndata) == USD8.UnauthorizedTreasury.selector);
        usd8.mint(recipient, mintAmount);
        assert(usd8.totalSupply() == supplyBefore + mintAmount);
        assert(usd8.balanceOf(recipient) == balanceBefore + mintAmount);
    }

    function test_treasuryIsOneTimeAndBurnAuthorityStaysCanonical(
        address holder,
        address unrelatedHolder,
        uint128 holderBalanceSeed,
        uint128 unrelatedSupplySeed,
        ...
        (bool replacementSuccess, bytes memory replacementReturndata) = otherCaller.callBurn(usd8, holder, burnAmount);
        assert(!replacementSuccess);
        assert(_selector(replacementReturndata) == USD8.UnauthorizedTreasury.selector);
        assert(usd8.totalSupply() == supplyBefore);
        assert(usd8.balanceOf(holder) == balanceBefore);

...
contract TreasuryStrategyFlowsKontrolTest is Test {
    ...
    function test_honestManualWithdrawalUsesExactActualDeltaAndPreservesReserve(uint64 amount) public {
        ...
        // [C:EVENT_ORDERING_FOUNDRY] The reserve token emits Transfer before
        // Treasury emits WithdrawnFromStrategy. Kontrol v1.0.255 cannot advance
        // expectEmit past that prior log; exact multi-log order is a Foundry regression.
    ...
    function test_shortWithdrawalIsAcceptedAndAccountedByActualDelta(uint64 amount) public {
        vm.assume(amount > 1);
        TreasuryStrategyFlowsStrategy strategy = _newApprovedStrategy();
        usdc.mint(address(treasury), amount);
        treasury.depositToStrategy(strategy, amount);

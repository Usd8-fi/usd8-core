...
contract TreasuryInitUpgradeFactory {
    function deploy(Registry registry_, IERC20 reserve_) external returns (address) {
        return
            address(
                new ERC1967Proxy(address(new Treasury()), abi.encodeCall(Treasury.initialize, (registry_, reserve_)))
            );
    }
}
...
contract TreasuryInitUpgradeKontrolTest is Test {
    ...
    bytes32 internal constant HARVEST_BUFFER_DIVISOR_SLOT = bytes32(uint256(TREASURY_STORAGE) + 1);
    uint256 internal constant REPRESENTATIVE_DIVISOR = 777;
    ...
    function _seedRepresentativeState() internal returns (RepresentativeState memory state) {
        ...
        treasury.setHarvestBufferDivisor(REPRESENTATIVE_DIVISOR);
    ...
    function _snapshotRepresentativeState() internal view returns (RepresentativeState memory state) {
        ...
        state.harvestBufferDivisor = treasury.harvestBufferDivisor();
    ...
    function _assertRepresentativeState(
        ...
        assert(actual.harvestBufferDivisor == expected.harvestBufferDivisor);
    ...
    function test_freshZeroDivisorStorageFallsBackToDefault() public view {
        assert(vm.load(address(treasury), HARVEST_BUFFER_DIVISOR_SLOT) == bytes32(0));
        assert(treasury.harvestBufferDivisor() == treasury.HARVEST_BUFFER_DIVISOR());
    }

    function test_legacyZeroDivisorStorageFallsBackToDefaultWithoutChangingPriorState() public {
        treasury.setHarvestBufferDivisor(REPRESENTATIVE_DIVISOR);
        address registryBefore = address(treasury.registry());
        address reserveBefore = address(treasury.USDC());
        address usd8Before = address(treasury.usd8());
        vm.store(address(treasury), HARVEST_BUFFER_DIVISOR_SLOT, bytes32(0));

        assert(vm.load(address(treasury), HARVEST_BUFFER_DIVISOR_SLOT) == bytes32(0));
        assert(treasury.harvestBufferDivisor() == treasury.HARVEST_BUFFER_DIVISOR());
        assert(address(treasury.registry()) == registryBefore);
        assert(address(treasury.USDC()) == reserveBefore);
        assert(address(treasury.usd8()) == usd8Before);
        assert(treasury.strategiesLength() == 0);
        assert(treasury.profitReceiversLength() == 0);
    }
    ...
    function test_initializeRejectsSymbolicWrongReserveDecimals(uint8 wrongDecimals) public {
        ...
        // [C:EXTERNAL_DECIMALS] Keep the reserve bytecode concrete while making
        // the external decimals() response symbolic; symbolic immutables make
        // Kontrol's valid-jumpdest computation depend on symbolic creation code.
        TreasuryInitUpgradeWrongDecimals wrong = new TreasuryInitUpgradeWrongDecimals(5);
        vm.mockCall(address(wrong), abi.encodeWithSignature("decimals()"), abi.encode(wrongDecimals));
        TreasuryInitUpgradeFactory factory = new TreasuryInitUpgradeFactory();
        (bool success, bytes memory returndata) =
            address(factory).call(abi.encodeCall(TreasuryInitUpgradeFactory.deploy, (registry, IERC20(address(wrong)))));

        assert(!success);
        assert(returndata.length == 36);
        assert(_selector(returndata) == Treasury.InvalidReserveDecimals.selector);
        uint256 encodedDecimals;
        assembly {
            encodedDecimals := mload(add(returndata, 0x24))
        }
        assert(encodedDecimals == wrongDecimals);
    }
    ...
    function test_usd8TopologyIsOneTimeAndTreasuryResolutionStaysCanonical() public {
        address registryBefore = address(treasury.registry());
        address reserveBefore = address(treasury.USDC());
        USD8 replacement =
        ...
        (bool success, bytes memory returndata) =
            address(registry).call(abi.encodeCall(Registry.setUsd8, (address(replacement))));

        assert(!success);
        assert(_selector(returndata) == Registry.Usd8AlreadySet.selector);
        assert(registry.usd8() == address(usd8));
        assert(address(treasury.usd8()) == address(usd8));
    ...
    // [C:ADDRESS_REPRESENTATIVE] Upgrade authorization depends only on equality
    // with the Registry timelock, so OUTSIDER represents every non-timelock caller.
    function test_nonTimelockCannotUpgradeAtomically() public {
        ...
        assert(!success);
        _assertExactRevert(returndata, abi.encodeWithSelector(Registry.UnauthorizedTimelock.selector, OUTSIDER));

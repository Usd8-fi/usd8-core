...
contract TreasurySweepKontrolTest is Test {
    ...
    function test_registryUsd8IsOneTimeCanonicalAndReplacementRemainsSweepable(uint64 oldUsdc, uint64 newUsdc) public {
        ...
        (bool setSuccess, bytes memory setReturndata) =
            address(registry).call(abi.encodeCall(Registry.setUsd8, (address(replacement))));
        assert(!setSuccess);
        assert(_selector(setReturndata) == Registry.Usd8AlreadySet.selector);
        assert(registry.usd8() == address(usd8));

        vm.assume(newUsdc > 0);
        uint256 newAmount = uint256(newUsdc) * treasury.USDC_TO_USD8_SCALE();
        vm.prank(address(treasury));
        replacement.mint(address(treasury), newAmount);

        (bool protectedSuccess, bytes memory returndata) =
            address(treasury).call(abi.encodeCall(SharedBase.sweepToken, (IERC20(address(usd8)), RECIPIENT)));
        assert(!protectedSuccess);
        assert(_selector(returndata) == SharedBase.NothingToSweep.selector);
        assert(usd8.balanceOf(address(treasury)) == oldAmount);

        treasury.sweepToken(IERC20(address(replacement)), RECIPIENT);
        assert(replacement.balanceOf(address(treasury)) == 0);
        assert(replacement.balanceOf(RECIPIENT) == newAmount);
        assert(usd8.balanceOf(address(treasury)) == oldAmount);
    }
    ...
    // [C:ADDRESS_REPRESENTATIVE] OUTSIDER represents all non-admin callers;
    // sweep authorization depends only on Registry admin equality.
    function test_unauthorizedTokenSweepIsAtomic(uint128 amount) public {
        vm.assume(amount > 0);
        TreasurySweepStandardToken token = new TreasurySweepStandardToken("Foreign", "FRN");
        token.mint(address(treasury), amount);
    ...
    function test_unauthorizedEthSweepIsAtomic(uint128 amount) public {
        vm.assume(amount > 0);

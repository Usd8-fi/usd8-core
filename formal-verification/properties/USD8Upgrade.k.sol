...
contract USD8UpgradeKontrolTest is Test {
    ...
    /// @dev [C:ECDSA_CORRECT] ecrecover is modeled as returning `nonceOwner`;
    ///      this helper seeds nonce/allowance state for the upgrade-preservation
    ///      obligation and does not claim to prove ECDSA or keccak correctness.
    function _seedPermitNonce(uint256 value) internal returns (address nonceOwner) {
        nonceOwner = vm.addr(NONCE_OWNER_KEY);
        uint256 deadline = type(uint256).max;
        vm.mockCall(address(1), bytes(""), abi.encode(nonceOwner));
        usd8.permit(nonceOwner, SPENDER, value, deadline, 27, bytes32(uint256(1)), bytes32(uint256(1)));
    }

...
contract TreasuryReceiverSetKontrolTest is Test {
    ...
    function test_existingReceiverUpdateChangesOnlyWeightAndModeWithoutDuplicateOrReorder(bool hookMode) public {
        uint256 newWeight = 44;
        _seedThree();
    ...
    // [C:ADDRESS_REPRESENTATIVE] Receiver curation ACL depends only on equality
    // with Registry admin/timelock, so OUTSIDER covers every unauthorized caller.
    function test_unauthorizedSetAndRemoveAreAtomic(uint64 weight, bool hookMode) public {
        _seedThree();

        (bool setSuccess, bytes memory setData) =
            _callAs(OUTSIDER, abi.encodeCall(Treasury.setProfitReceiver, (MISSING, uint256(weight), _mode(hookMode))));
        assert(!setSuccess);
        assert(_selector(setData) == Registry.UnauthorizedAdmin.selector);

        (bool removeSuccess, bytes memory removeData) =
            _callAs(OUTSIDER, abi.encodeCall(Treasury.removeProfitReceiver, (RECEIVER_B)));
    ...
    function test_adminAndTimelockCanBothCurateReceivers() public {
        uint256 adminWeight = 11;
        uint256 timelockWeight = 22;
        vm.prank(ADMIN);

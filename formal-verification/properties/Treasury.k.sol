...
contract TreasuryKontrolTest is Test {
    ...
    function test_usd8ToUsdcRateZeroSupplyRevertsEvenWithDonatedReserve(uint64 donatedUsdc) public {
        usdc.mint(address(treasury), donatedUsdc);
        (bool success, bytes memory returndata) =
            address(treasury).staticcall(abi.encodeCall(Treasury.usd8ToUsdcRate, ()));
        assert(!success);
        assert(_selector(returndata) == Treasury.NoUsd8Supply.selector);
        assert(usd8.totalSupply() == 0);
        assert(treasury.getReserveBalance() == donatedUsdc);
    }

    function test_usd8ToUsdcRateCapsHealthyAndSurplusReserveAtOneWad(uint64 mintedUsdc, uint64 surplusUsdc) public {
        vm.assume(mintedUsdc > 0);
        vm.assume(uint256(mintedUsdc) + surplusUsdc <= type(uint64).max);
        _mintToUser(mintedUsdc);
        usdc.mint(address(treasury), surplusUsdc);
        assert(treasury.getReserveBalance() * SCALE >= usd8.totalSupply());
        assert(treasury.usd8ToUsdcRate() == 1e18);
    }

    function test_usd8ToUsdcRateMatchesExactDistressedReserveRatio(uint64 mintedUsdc, uint64 lossUsdc) public {
        _makeDistressed(mintedUsdc, lossUsdc);
        uint256 reserveInUsd8 = treasury.getReserveBalance() * SCALE;
        uint256 supply = usd8.totalSupply();
        assert(reserveInUsd8 < supply);
        assert(treasury.usd8ToUsdcRate() == Math.mulDiv(reserveInUsd8, 1e18, supply));
    }
}
